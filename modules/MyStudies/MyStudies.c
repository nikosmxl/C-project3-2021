...
bool ms_insert_student(Student student){
    ...
    if (to_return == 1){
        ...
        if (cn2 == NULL){
            ...
            set_insert(studies->cities2, student);
...
bool ms_remove_student(int id){
    ...
    set_remove(studies->cities, stud);

    set_remove(studies->years, stud);

    int old_size = set_size(studies->students);
    set_remove(studies->students, stud);

    studies->count -= set_size(studies->students) - old_size;

    return (set_size(studies->students) - old_size);
}

Student ms_get_student(int id){
    return set_find(studies->students, &id);
}
...
List ms_top_by_year(int year, int number){
    ...
    for (int i = 1 ; i <= number ; i++){
        list_insert_next(to_return, LIST_EOF, set_get_at(set, set_size(set) - i));
    }
...
List ms_bottom_by_year(int year, int number){
    ...
    for (int i = 0 ; i < number ; i++){
        list_insert_next(to_return, LIST_EOF, set_get_at(set, i));
    }
